...
namespace Agricultura_práctica_final
{
    internal class CProduccion
    {
        ...
        public long BuscarProduccion(int localidad, int cultivo)
        {
            ...
            DataRow prod = DS.Tables["Produccion"].Rows.Find(valores); // buscar el registro en producción

            if (prod != null)
            { 
                produccion = (int)prod["produccion"]; //// si existe obtener el valor de producción
            }
            return produccion;
        }
